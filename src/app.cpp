...
void App::run()
{
    ...
    if(SDL_Init(SDL_INIT_VIDEO) == 0) // 初始化视频子系统
    {
        ...
        if(m_window == nullptr) return; // 创建窗口失败
        if(!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)) return; // 选择png图片初始化
        ...
        engine.initModules();
        engine.getRenderer()->loadTexture(m_window); // 贴图加载
        engine.getRenderer()->loadFont(); // 字体加载

        m_app_state = new Menu; // 创建菜单栏
        ...
        while(is_running)
        {
            ...
            if(m_app_state->finished()) // 确定开始游戏 or 退出
            {
                AppState* new_state = m_app_state->nextState();
                delete m_app_state;
                m_app_state = new_state;
            }
            if(m_app_state == nullptr) break; // 退出游戏
...
void App::eventProces()
{
    ...
    while(SDL_PollEvent(&event))
    {
        ...
        m_app_state->eventProcess(&event); // 根据事件更新状态
    }
}
