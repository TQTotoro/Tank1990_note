...
Object::Object(double x, double y, SpriteType type)
{
    ...
    dest_rect.x = pos_x;
    dest_rect.y = pos_y;
    dest_rect.h = m_sprite->rect.h; // 尺寸无变化
    dest_rect.w = m_sprite->rect.w; // 尺寸无变化
...
void Object::draw()
{
    ...
    Engine::getEngine().getRenderer()->drawObject(&src_rect, &dest_rect); // 将对象数据传至显示器
}

void Object::update(Uint32 dt) // 更新同一类对象的不同状态
{
    if(to_erase) return;
...
SDL_Rect Object::moveRect(const SDL_Rect &rect, int x, int y) // 对象的位置移动
{
    SDL_Rect r;
    r.x = rect.x + x*rect.w;
    r.y = rect.y + y*rect.h;
...
SDL_Rect intersectRect(SDL_Rect *rect1, SDL_Rect *rect2) // 返回两矩形的交集
{
