...
void Renderer::loadTexture(SDL_Window* window)
{
    ...
    m_renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC); // 硬件加速 + 和显示器的刷新率同步

    std::string base_path = "resources/png/";
    surface = IMG_Load((base_path + AppConfig::texture_path).c_str()); // 转成C风格字符串

    //load surface
    if(surface != nullptr && m_renderer != nullptr)
        m_texture = SDL_CreateTextureFromSurface(m_renderer, surface);

    SDL_FreeSurface(surface); // 只需要texture, 不需要surface, 二者存储的数据相同
}

void Renderer::loadFont()
{
    std::string base_path = "resources/font/";
    m_font1 = TTF_OpenFont((base_path + AppConfig::font_name).c_str(), 28);
    m_font2 = TTF_OpenFont((base_path + AppConfig::font_name).c_str(), 14);
    m_font3 = TTF_OpenFont((base_path + AppConfig::font_name).c_str(), 10);
}

void Renderer::clear()
{
    SDL_SetRenderDrawColor(m_renderer, 110, 110, 110, 255); // 灰色模式
    SDL_RenderClear(m_renderer); // 清除渲染
}

void Renderer::flush()
{
    SDL_RenderPresent(m_renderer); // 展现渲染效果
}
